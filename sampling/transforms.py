...
class MinMaxScaler(object):
    ...
    def transform(self, X, y=None):
        if np.any(X!=0):
			...
			#X *= scale_
            X = np.multiply(X, scale_, out=X, casting='unsafe')
            X = np.add(X, min_, out = X, casting='unsafe')
        ...
        if y is None:
            ...
            if np.any(y!=0):
                ...
                y = np.multiply(y, scale_, out=y, casting='unsafe')
                y = np.add(y, min_, out = y, casting='unsafe')
...
class RandomAffine(object):
    ...
    def __init__(self, 
        ...
        if flip_range:
            flip_tform = Randomflip(flip_range, fill_mode=fill_mode, fill_value=fill_value, lazy=True)
...
class Randomflip(object):
    ...
    def __init__(self, 
        ...
        if not isinstance(flip_range, list) and not isinstance(flip_range, tuple):
            raise ValueError('flip_range must be tuple or list with 2 values')
    ...
    def transform(self, X, y=None):
        fx = random.randint(self.flip_range[0], self.flip_range[1])
        fy = random.randint(self.flip_range[0], self.flip_range[1])
        self._flip = (fx,fy)
        flip_matrix = np.array([[0, 0, fx],
                                [0, fy, 0],
                                [1, 0, 0]])
        if self.lazy:
            return flip_matrix
        else:
            x_transformed = apply_transform(X, flip_matrix, 
                                            fill_mode=self.fill_mode, 
                                            fill_value=self.fill_value)
            if y is not None:
                y_transformed = apply_transform(y, flip_matrix,
                                                fill_mode=self.target_fill_mode, 
                                                fill_value=self.target_fill_value)
                return x_transformed, y_transformed
            else:
