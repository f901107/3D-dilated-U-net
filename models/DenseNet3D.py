...
from keras.layers import Activation, AveragePooling3D,BatchNormalization, Conv3D, Conv3DTranspose, Dropout, GlobalAveragePooling3D, GlobalMaxPooling3D
from keras.layers import Input, MaxPooling3D, Reshape, UpSampling3D, concatenate, Dense
...
def DenseNet3D_FCN(input_shape, nb_dense_block=3, growth_rate=8, nb_layers_per_block=4, reduction=0.0, dropout_rate=0.0, weight_decay=1E-4,
                   init_conv_filters=48, include_top=True, input_tensor=None, classes=1, activation='sigmoid', upsampling_conv=128,init_lr=0.001,
                   upsampling_type='deconv', early_transition=False, transition_pooling='max', initial_kernel_size=(5, 5, 5)):
    ...
    model = model.compile(optimizer=opt.Adam(lr=init_lr), loss=loss_dice_coefficient_error, metrics=[dice_coefficient])
    return model
